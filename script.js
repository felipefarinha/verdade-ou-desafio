...
function set_category(){
  if(nivel == li_infantil.innerText) {
    ...
    max = (base_perguntas_infantil.length + base_perguntas_adolescente.length) + base_perguntas_adulto.length;
    count_adulto++
  }
};
...
function remove_frase_do_array(){
  ...
  if(nivel == li_infantil.innerText){
    ...
    base_perguntas_adulto.splice(frase_do_array, 1);
  }
};
...
function back_text_array(){
  ...
  if(base_perguntas_infantil.length == 0){
    ...
    base_perguntas_adulto = arr_excluidos
    arr_excluidos = [];
  }
};
